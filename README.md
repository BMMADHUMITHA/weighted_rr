...
The program continuously reads PF0 and PF4: switch_value = GPIO_PORTF_DATA_R & 0x11;
...
The variable:
uint8_t speed = 0; stores the current speed index.
...
uint8_t led_state = 0;
The counter is incremented continuously: blink_counter++;

When the counter reaches the selected speed value:
if (blink_counter >= speeds[speed])
the counter is reset: blink_counter = 0;
...
the speed index is incremented: speed++;
...
The switch is then marked as already processed: sw1_state = 2;
...
the colour index is incremented: colour++;
